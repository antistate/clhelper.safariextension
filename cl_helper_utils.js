...
function imagePreview(e, a) {
    ...
    for (var c = 0; c < b; c++) {
		if (a[c].indexOf('thumb') == -1) {
			d.append("<div class='ziink' style='position: relative; float: left;'>" + a[c].replace('org', 'org/thumb').replace('alt',
			'style="width:' + options.tnMaxWidth + 'px; height:'+ options.tnMaxHeight + 'px;" alt') + '</div> ');
			c = c + 1;
		}
		else {
			d.append("<div class='ziink' style='position: relative; float: left;'>" + a[c].replace('alt',
			'style="width:' + options.tnMaxWidth + 'px; height:'+ options.tnMaxHeight + 'px;" alt') + '</div> ');
		}
    }
        ...
        if (e) {
            ...
            if (options.showTn) {
				...
				if (d = e.match(/<img src=\"http:\/\/images\.craigslist\.org\/thumb\/([^>]+)\">/gi)) {
					...
					if (d = e.match(/<img id=\"iwi\" src=\"http:\/\/images\.craigslist\.org\/([^>]+)\">/gi))
						imagePreview(b, d);
                }
            }
            if (clAdPreview) {
